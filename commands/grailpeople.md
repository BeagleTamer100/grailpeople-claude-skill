...
- A **product** ("is X worth it", a specific product name) → `get_product_evidence`
- A **brand** ("is X good", "X overhyped") → `get_brand_intel`
- A **creator** ("what does X recommend") → `get_creator_picks`
- A **need** rather than a name ("best serum for redness") → `search_products`, decomposed into
  `ingredient=` / `concern=` / `subcategory=` rather than one big `q=`
- If the exact brand or creator name is uncertain → `resolve_entity` first, then the tool above
